...
export default function AdminLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/auth/globe-login`, {
        method: "POST",
      ...
      if (data.success) {
        localStorage.setItem("globeToken", data.token);
        localStorage.setItem("adminUser", JSON.stringify(data.user));
        localStorage.removeItem("adminActiveSection");
        
        if (data.user.role === "globe") {
          router.push("/globe");
        } else {
          setError("Unauthorized access. This portal is for Globe team only.");
          localStorage.clear();
        }
      } else {
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div style={{ textAlign: "center", marginBottom: "32px" }}>
          ...
          <h1 style={{ fontSize: "1.8rem", fontWeight: 900, marginTop: "16px" }}>Globe Portal</h1>
          <p style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>Secure access for Globe team only.</p>
        </div>
