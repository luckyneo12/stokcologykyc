...
function isVideo(src) {
  return src?.startsWith("data:video/") || src?.toLowerCase().endsWith(".mp4") || src?.toLowerCase().endsWith(".webm") || src?.includes("/video/upload/");
}
      ...
      {isPdf(src) && shouldDisplayAsIframe(label) ? (
        ...
        >
          <div style={{
            ...
            {isPdf(src) ? <PdfThumbnail src={src} /> : isVideo(src) ? <video src={src} controls autoPlay loop muted style={{ maxHeight: "100%", maxWidth: "100%", objectFit: "contain", borderRadius: 4, pointerEvents: "auto" }} /> : <img src={src} alt="Preview" style={{ maxHeight: "100%", maxWidth: "100%", objectFit: "contain", borderRadius: 4 }} />}
          </div>
        </div>
      )}
      {!(isPdf(src) && shouldDisplayAsIframe(label)) && (
    ...
    <div style={{ display: "flex", width: "100vw", height: "100vh", overflow: "hidden", background: "var(--bg-secondary)" }}>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 20px", background: "var(--bg-primary)", borderBottom: "1px solid var(--border-color)", boxShadow: "0 4px 24px rgba(0,0,0,0.02)" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 24 }}>
          ...
          <button onClick={() => setShowRejectionConfirmModal(true)} disabled={submitting} style={{ padding: "6px 12px", borderRadius: 6, border: "1px solid #fecaca", background: "#fef2f2", color: "#ef4444", fontWeight: 600, fontSize: "0.8rem", cursor: submitting ? "not-allowed" : "pointer", opacity: submitting ? 0.6 : 1, display: "flex", alignItems: "center", gap: 6 }}>
      ...
      <div style={{ display: "flex", flex: 1, overflow: "hidden" }}>
        ...
        <div style={{ width: "30%", background: "var(--bg-primary)", borderRight: "1px solid var(--border-color)", display: "flex", flexDirection: "column" }}>
          ...
          <div style={{ flex: 1, overflowY: "auto", paddingBottom: "60vh" }}>
            {unlockedSteps.map((step) => {
              ...
              return (
                <div id={`module-${step.id}`} key={step.id} style={{ borderBottom: "1px solid var(--border-color)" }}>
                  <div 
