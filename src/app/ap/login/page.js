...
export default function APLogin() {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/auth/ap-login`, {
        method: "POST",
      ...
      if (data.success) {
        localStorage.setItem("apToken", data.token);
        localStorage.setItem("apUser", JSON.stringify(data.user));
        
        if (data.user.role === "ap") {
          router.push("/ap");
        } else {
          setError("Unauthorized access. This portal is for Associate Partners only.");
    ...
    <div style={{ 
      ...
      <div style={{ 
        ...
        <div style={{ textAlign: "center", marginBottom: "32px" }}>
          ...
          <h1 style={{ fontSize: "1.8rem", fontWeight: 900, marginTop: "16px" }}>AP Portal</h1>
          <p style={{ color: "var(--text-muted)", fontSize: "0.9rem" }}>Associate Partner login portal.</p>
        </div>
