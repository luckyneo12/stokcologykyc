  ...
  const generatePdf = async () => {
    try {
      ...
      try {
        ...
        const tplRes = await fetch(`${API_URL}/api/admin/pdf-templates/active`, {
          headers: {
        ...
        if (tplRes.ok) {
          ...
          if (tplData && tplData.fields) {
            templateFields = typeof tplData.fields === "string" ? JSON.parse(tplData.fields) : tplData.fields;
            if (tplData.basePdfUrl) {
              if (tplData.basePdfUrl.startsWith('public/')) {
                basePdf = '/' + tplData.basePdfUrl.replace('public/', '');
              } else {
                basePdf = getFullUrl(tplData.basePdfUrl);
              }
            }
          }
        }
      } catch (err) {
        console.warn("Could not fetch PDF template, falling back to default.", err);
      }

      // 2. Fetch the base PDF
      ...
      const variables = {
        ...
        plan: "Standard",
        fullName: personalDetails?.fullName || ocrData?.name,
        fatherName: personalDetails?.fatherName,
        motherName: personalDetails?.motherName,
        gender: personalDetails?.gender,
        dob: personalDetails?.dob || ocrData?.dob,
        nationality: personalDetails?.nationality || "Indian",
        maritalStatus: personalDetails?.maritalStatus,
        occupation: personalDetails?.occupation,
        annualIncome: personalDetails?.annualIncome || personalDetails?.incomeRange,
        pan: identityDetails?.pan,
        aadhaar: identityDetails?.aadhaar,
        ...
        selfie: photoUrl,
        signature: sigUrl,
        esign: null, // Placeholder for later
        panImage: getFullUrl(panUpload?.filePreview || identityDetails?.panPath || ocrData?.pan_path),
        aadhaarImage: getFullUrl(documents?.frontPreview || documents?.front || ocrData?.aadhaar_front_path || identityDetails?.aadhaarFront)
      };
      ...
      // If we have template fields, apply them
      if (templateFields.length > 0) {
        const pages = pdfDoc.getPages();
        
        for (const field of templateFields) {
          // pdf-lib pages are 0-indexed, builder is 1-indexed
          ...
          // PDF coordinates: (0,0) is bottom-left. Builder coordinates: (0,0) is top-left
          // For text, y is baseline. We subtract the fontSize as an approximation for the height.
          // For images, y is bottom-left.
          const pdfY = pageHeight - field.y - (field.type === 'image' ? field.height : (field.fontSize || 12));

          const mappedValue = variables[field.variable] || field.matchValue || "";

          if (field.type === "text") {
            if (mappedValue) {
              page.drawText(String(mappedValue), {
                x: field.x,
                y: pdfY,
            ...
            if (String(mappedValue).toLowerCase() === String(field.matchValue).toLowerCase()) {
              page.drawText("✓", {
                x: field.x,
                y: pdfY,
            ...
            if (mappedValue) {
              try {
                const bytes = await fetchImage(mappedValue);
                if (bytes) {
                  let img;
                  try {
                    const isPng = mappedValue.toLowerCase().includes(".png") || mappedValue.startsWith("data:image/png");
                    img = isPng ? await pdfDoc.embedPng(bytes) : await pdfDoc.embedJpg(bytes);
                  } catch (embedErr) {
                    console.warn(`Failed first embed attempt for ${field.variable}, converting to PNG fallback...`, embedErr);
                    const pngBytes = await convertToPngBytes(mappedValue);
                ...
                console.warn(`Image embed failed for ${field.variable}:`, imgErr);
              }
            }
          }
        }
      } else {
        // FALLBACK: If no template fields, do the old Annexure logic
      ...
      for (const docItem of allDocs) {
        try {
          ...
          console.warn(`Failed to process document ${docItem.name}:`, docErr);
