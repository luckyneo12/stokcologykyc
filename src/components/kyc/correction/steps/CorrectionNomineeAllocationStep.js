...
export default function CorrectionNomineeAllocationStep({ stepId }) {
  const { applicationData, drafts, saveDraft, nextCorrectionStep, prevCorrectionStep, addToast } = useCorrection();
  
  // Nominees can be freshly edited in drafts, or fallback to original applicationData
  const nominees = drafts.nomineeChoice?.nominees || drafts.nomineeDetails?.nominees || applicationData?.nomineeDetails?.nominees || [];
  
  // Initial allocation from draft or original app data
  const initialAllocation = drafts.nomineeAllocation?.percentages 
    ? drafts.nomineeAllocation 
    : applicationData?.nomineeAllocation;
    ...
    <div className="container-sm" style={{ paddingTop: "2vh", paddingBottom: "4vh", maxWidth: "500px" }}>
      ...
      <div className="card animate-slide-up" style={{ padding: "32px", borderRadius: "32px", border: "1.5px solid var(--border-color)", background: "var(--bg-card)" }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <button className="btn btn-secondary" onClick={prevCorrectionStep} style={{ width: "100%", height: "56px", borderRadius: "16px", fontSize: "1.1rem", fontWeight: 900 }}>
            Back
          </button>
        </div>
